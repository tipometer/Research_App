...
server/
├── _core/
│   └── llm.ts                  ← TÖRÖLVE (Manus-specifikus helper; nincs deprecated alias)
└── ai/
...
**Nincsenek.** A `modelRouting`, `aiConfigs`, `sources` táblák már léteznek a schema-ban és megfelelőek.

**Megjegyzés a `sources.publishedAt`-ről**: a séma (`drizzle/schema.ts:83`) `varchar("publishedAt", { length: 32 })` — nullable string oszlop (nem timestamp). Amikor C1-ben a grounding nem ad dátumot, a string mező `NULL` marad (varchar default nullable). **Nincs típus-migráció, nincs oszlop-módosítás.** A UI csak a `null` literal-t kell hogy kezelje a display logikában (§6.6).

**Provider enum**: `aiConfigs.provider` értékkészlete `["openai", "anthropic", "gemini"]` (`drizzle/schema.ts:131`). Az egész kódbázisban ezeket a string értékeket használjuk a provider azonosításra (lásd §4.2) — a **Vercel AI SDK csomagneve** (`@ai-sdk/google`) és a **belső provider azonosító** (`"gemini"`) tudatosan eltér; az SDK hívásnál `createGoogleGenerativeAI(...)` használatos, a DB lookup-nál `provider = "gemini"`.
...
A visszaadott értékek **pontosan** megegyeznek a `aiConfigs.provider` enum értékeivel (`["openai", "anthropic", "gemini"]`), hogy a DB lookup közvetlenül működjön:

```typescript
type ProviderId = "openai" | "anthropic" | "gemini";

function detectProvider(modelName: string): ProviderId {
...
**Fontos**: a `"gemini"` string a belső provider azonosító (DB enum + ENV kulcs név `GEMINI_API_KEY`). A Vercel AI SDK oldalon a csomag neve `@ai-sdk/google` és a factory `createGoogleGenerativeAI(...)` — ez a szándékos eltérés a Google marketing elnevezés vs. a modell-család közti különbség miatt. A `providers.ts` wrapper elrejti ezt.
...
```typescript
lookupApiKey(provider: ProviderId) =
  db.select().from(aiConfigs).where(and(eq(provider, X), eq(isActive, true))).apiKey
  ?? ENV[`${provider.toUpperCase()}_API_KEY`]  // OPENAI_API_KEY, ANTHROPIC_API_KEY, GEMINI_API_KEY
...
**Modellnév-konvenciók**: a táblában szereplő értékek a rövid (családi) modellnevek. Az implementáció során a **Vercel AI SDK által elfogadott pontos API modell-azonosítót** kell beírni, amely általában dátum suffix-szel pontos verzió (pl. Anthropic: `claude-sonnet-4-6-20251001`; OpenAI: `gpt-4.1-mini-2025-04-14`; Google Gemini: `gemini-2.5-flash`). Az admin UI Model Routing tabján az admin kézzel írhatja be a pontos API nevet — ezek a seed alapértékek csak indikatívak; az implementációs ticket első feladata, hogy megnézze a provider dokumentációkat és bevezesse az aktuális verzionált modellnevet a `seed.ts`-be.
...
**URL decoding megjegyzés**: bizonyos Gemini redirect payload-ok base64 után még percent-encoded (URL-encoded) karaktereket tartalmaznak. Ha a decoded string tartalmaz `%` jelet és a `decodeURIComponent()` nem dob hibát, érdemes futtatni:

```typescript
if (decoded.includes("%")) {
  try { decoded = decodeURIComponent(decoded); } catch { /* hagyjuk raw-ban */ }
}
```

Integration tesztben ellenőrizendő a valós Gemini response-ok alapján.
...
export const SynthesisSchema = z.object({
  ...
  reportMarkdown: z.string().min(4000),  // ~800 szó minimum (karakter-alapú ellenőrzés)
  verdictReason: z.string().min(50).max(500),
});
```

**Megjegyzés `reportMarkdown.min(4000)`-hez**: a PRD 800 szó minimumot specifikál. 800 szó ≈ 4000-5000 karakter magyarul/angolul. Ezt karakter-alapon validáljuk Zod-ban (szószámot a Zod natívan nem támogat); ha szigorúbb ellenőrzés kell, a `pipeline-phases.ts`-ben egy post-hoc `split(/\s+/).length >= 800` check beépíthető, ami Zod error-t dob, ha nem éri el.
...
**Timeout és retry interakciója**: a per-phase timeout a **teljes budget** (initial hívás + esetleges Zod retry együtt). Ha az initial hívás elhasználja a timeout >50%-át és Zod error-ral fail-el, a `invokeWithRetry` **nem próbálkozik újra** — azonnal provider error ágra esik. A reasoning: Zod retry ~20-30s plusz hívást jelent; ha <30s maradt a phase budget-ből, értelmetlen elindítani, mert úgyis timeout-olni fog. A döntés egy `remainingMs = deadline - Date.now()` check-kel történik a retry előtt.
...
Cél: **megfelelő egység teszt coverage** a router lookup logikára, a grounding extraction-re, a classify heurisztikára és a Zod sémákra. Nincs rögzített teszt-szám kvóta — a fókusz a meaningful cases (happy path + boundary + hiba-ág) lefedése, nem a számon tartás.
...
| 3 | `server/routers.ts` polling + brainstorm tRPC procedure-k — `invokeLLM` hívások cseréje `llmRouter.invoke("polling", ...)` / `llmRouter.invoke("brainstorm", ...)`-ra, `PollingSchema` / `BrainstormSchema` használatával | Brainstorm ötletgenerálás és Survey kérdésgenerálás új SDK-n, admin routing-on keresztül |
...
| 4 | Meglévő 17 vitest teszt frissítése az új router API-ra (`invokeLLM` mock → `llmRouter.invoke` mock) | Regression nem töri a CI-t |
| 4 | Integration tesztek min. 1 fázisonként (mind a 6: 1-4 + polling + brainstorm), `test.skip` alap | `pnpm test:integration` fut manuálisan |
